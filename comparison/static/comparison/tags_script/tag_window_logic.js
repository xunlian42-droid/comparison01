...
document.addEventListener("DOMContentLoaded", () => {
    (async () => {
        ...
        const url = window.workPageMapUrl || 'comparison/static/comparison/workPageMap.json';
        ...
        function openInParentAndGotoByInfo(pageInfo, fallbackKey) {
            const targetPage = (pageInfo && pageInfo.page) ? pageInfo.page : (window.currentPageFilename ? window.currentPageFilename() : '');
            const anchor = (pageInfo && pageInfo.anchor) ? pageInfo.anchor : fallbackKey;
            const encodedAnchor = encodeURIComponent(anchor);
            const normalizedPage = targetPage.startsWith('/') ? targetPage : '/' + targetPage;
            const absoluteUrl = `${window.location.origin}${normalizedPage}#${encodedAnchor}`;

            const openerExists = window.opener && !window.opener.closed;
            if (!openerExists) {
                // 親がないなら自分で遷移
                // window.location.href = `${targetPage}#${encodedAnchor}`;
            ...
            try {
                ...
                if (openerPage === targetPage) {
                    // 同じページなら postMessage で確実にスクロール
                    window.opener.postMessage({ action: 'goto', anchor }, window.location.origin);
                    window.close();
                    return;
                }

                // 別ページなら親を遷移させる（ハッシュ付き）
                //window.opener.location.href = `${targetPage}#${encodedAnchor}`;
        ...
        function showTagResults(tagKey, workId) {
            ...
            entries.forEach(({ id, title }) => {
                ...
                a.addEventListener("click", e => {
                    ...
                    // workPageMap の読み込み完了を待ってから処理
                    window._workPageMapLoaded.then(() => {
                        // workPageMap のキーは外部IDや tr の id と一致させること
                        const key = id || title;
                        const pageInfo = (window.workPageMap && window.workPageMap[key]) ? window.workPageMap[key] : null;

                        const targetPage = (pageInfo && pageInfo.page) ? pageInfo.page : (window.currentPageFilename ? window.currentPageFilename() : '');
                        const normalizedPage = targetPage.startsWith('/') ? targetPage : '/' + targetPage;
                        const absoluteUrl = `${window.location.origin}${normalizedPage}#${encodedAnchor}`;

                        // 親に scrollToTitle を期待していた従来処理は残すが、先に親とページ比較する新処理へフォールバック
                        if (window.opener) {
                            openInParentAndGotoByInfo(pageInfo, key);
                        } else {
                            // 親がない場合は自ウィンドウで遷移（フォールバック）
                            // const targetPage = (pageInfo && pageInfo.page) ? pageInfo.page : (window.currentPageFilename ? window.currentPageFilename() : '');
                            // const anchor = (pageInfo && pageInfo.anchor) ? pageInfo.anchor : key;
                            // window.location.href = `${targetPage}#${encodeURIComponent(anchor)}`;
                            window.location.href = absoluteUrl;
                        }
                    }).catch(() => {
                        // 読み込み失敗時のフォールバック
                        const key = id || title;
                        const anchor = key;
                        if (window.opener && typeof window.opener.scrollToTitle === 'function') {
                            // 既存処理（可能な限り）
                            window.opener.scrollToTitle(id);
                            window.close();
                        } else {
                            // window.location.href = `${window.currentPageFilename ? window.currentPageFilename() : ''}#${encodeURIComponent(anchor)}`;
                            window.location.href = absoluteUrl;
                        }
                    });
